...
func spawn():
	var player = get_node("/root/Player") as Player;
	var player2d = player.get_node("pokemon_rb_player")
	if is_offset:
		player2d.global_position += offset;
	else:
		player2d.global_position = global_position;
	player.spawned = true
